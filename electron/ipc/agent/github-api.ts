...
export async function fetchUnresolvedPrFeedback(
  ...
  try {
    const repoJsonGc = await execFileAsync('gh', ['repo', 'view', '--json', 'owner,name'], projectPath);
    ...
    const gcOutput = await execGraphQL(gcQuery, projectPath, 15000, { owner: repoInfoGc.owner.login, name: repoInfoGc.name, pr: prNumber });
  ...
  try {
    const repoOutput = await execFileAsync('gh', ['repo', 'view', '--json', 'nameWithOwner'], projectPath);
    ...
    const graphqlOutput = await execGraphQL(graphqlQuery, projectPath, 30000, { owner, repo, pr: prNumber });
    ...
    if (outdated.length > 0) {
      ...
      try {
        await resolveReviewThreads(projectPath, outdatedIds, taskId, projectName, q, getWindow);
...
export async function postThreadReplies(
  ...
  for (const reply of replies) {
    try {
      ...
      await execGraphQL(mutation, projectPath, 30000, { threadId: reply.threadId, body: reply.body });
      posted++;
    } catch (err) {
...
export async function resolveReviewThreads(
  ...
  for (const threadId of threadIds) {
    try {
      ...
      await execGraphQL(mutation, projectPath, 30000, { threadId });
      resolved++;
    } catch (err) {
...
export async function minimizeOldReviews(
  ...
  try {
    ...
    const output = await execGraphQL(query, projectPath, 15000, { owner: repoInfo.owner.login, name: repoInfo.name, pr: prNumber });
    ...
    for (const nodeId of idsToMinimize) {
      try {
        ...
        await execGraphQL(mutation, projectPath, 30000, { id: nodeId });
        minimized++;
      } catch {
...
export async function cleanupOldPRComments(
  ...
  try {
    ...
    const commitsOutput = await execFileAsync('git', ['log', '--format=%H', `${defaultBranch}..HEAD`], projectPath, 10000);
    ...
    for (const review of data.data.repository.pullRequest.reviews.nodes) {
      ...
      for (const comment of review.comments.nodes) {
        ...
        try {
          await execFileAsync('gh', ['api', '-X', 'DELETE', `repos/${repoSlug}/pulls/comments/${comment.databaseId}`], projectPath, 5000);
          deleted++;
        } catch {
          try {
            const mut = `mutation($id: ID!) { minimizeComment(input: {subjectId: $id, classifier: RESOLVED}) { minimizedComment { isMinimized } } }`;
            await execGraphQL(mut, projectPath, 30000, { id: comment.id });
            minimized++;
          } catch { /* skip */ }
        }
      }
      ...
      if (!review.isMinimized) {
        try {
          const mut = `mutation($id: ID!) { minimizeComment(input: {subjectId: $id, classifier: RESOLVED}) { minimizedComment { isMinimized } } }`;
          await execGraphQL(mut, projectPath, 30000, { id: review.id });
          minimized++;
        } catch { /* skip */ }
      }
    }
