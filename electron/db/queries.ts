...
export function createQueries(db: Database.Database) {
  return {
    ...
    insertProject: db.prepare(
      `INSERT INTO projects (id, name, path, repo, description, optional_skills, test_command, code_hosting, code_hosting_config, plugin_pm, plugin_pm_config, ai_agent, ai_agent_phases, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`
    ),
    updateProject: db.prepare(
      `UPDATE projects SET name=?, path=?, repo=?, description=?, optional_skills=?, test_command=?, code_hosting=?, code_hosting_config=?, plugin_pm=?, plugin_pm_config=?, ai_agent=?, ai_agent_phases=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`
    ),
